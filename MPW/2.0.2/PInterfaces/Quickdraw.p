...
{ THE FOLLOWING TYPES WERE ADDED FOR COLOR QUICKDRAW. }
	...
	ColorTable	= RECORD
					...
					ctSize: 	INTEGER;	{ number of entries - 1 in CTTable	}
					ctTable:	CSpecArray;
				END;
	...
	PixMap		= RECORD
					...
					rowBytes:	INTEGER;	{ offset to next line (high bit always set) }
	...
	GDevice 	= RECORD
					...
					gdSearchProc:	SProcHndl;	{ first element in search list	}
					gdCompProc: 	CProcHndl;	{ first element in complement list	}
...
PROCEDURE SeedCFill(srcBits, dstBits: BitMap; srcRect, dstRect: Rect;
	seedH, seedV: INTEGER; matchProc: ProcPtr; matchData: LONGINT);
	INLINE $AA50;
