...
// Build a table of pointers to functions in our linked library
{
	...
	GetIndString(&appName, 128, 1);
	err = GetSharedLibrary(&appName, kPowerPCArch, 0, &connID, &mainAddr, &errName);
	if (err) goto done;
	
	// Now, locate the table that is shared between the 68K code and the PowerPC code
	// (it's a pointer stored in a global variable)
	GetIndString(&tableName, 128, 2);
	err = FindSymbol(connID, &tableName, (Ptr*)&myTablePointer, &symClass);
