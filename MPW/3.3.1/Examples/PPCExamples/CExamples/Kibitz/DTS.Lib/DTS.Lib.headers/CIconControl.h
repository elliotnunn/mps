...
ControlHandle	CCIconNew(WindowPtr window, Rect *r, StringPtr title, Boolean vis, short val,
	...
	/* Create a new CIcon control.  All the parameters are what you would expect for creating
	** a new control. */

ControlHandle	CCIconNext(WindowPtr window, ControlHandle ctl, short dir, Boolean justActive);
	/* Return the next CIcon control from a window's control list. 
	**
	...
	** justActive:	only return active, visible CIcon controls.
	**
...
Boolean			IsCIconCtl(ControlHandle ctl);
	/* Check to see if the control is a CIcon control. */
