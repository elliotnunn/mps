...
{ 
	// prototypes for functions that don't belong to any one class
		pascal 	TerminalClikLoopProcPtr GetOldClikLoop(void);
		...
		// and try to create a vertical scroll control
			if ( good )
			{
				// set up TE record
					...
					fDocClik = (TerminalClikLoopProcPtr) (*fDocTE)->clickLoop;
					(*fDocTE)->clickLoop = (TEClickLoopUPP) ASMCLIKLOOP;
...
//-----------------------------------------------------------------------
// TEDocument::GetClikLoop -	
//
	TerminalClikLoopProcPtr TEDocument::GetClikLoop( void )
	{
		return fDocClik;
...
//-----------------------------------------------------------------------
// PascalClikLoop -	Gets called from our assembly language routine, AsmClikLoop,
...
//-----------------------------------------------------------------------
// ClikLoopProcPtr -	Gets called from our assembly language routine, AsmClikLoop,
//						which is in turn called by the TEClick toolbox routine. It
//						returns the address of the default clikLoop routine that was
//						put into the TERec by TEAutoView to AsmClikLoop so that it
//						can call it.
//
	pascal TerminalClikLoopProcPtr GetOldClikLoop(void)
	{
