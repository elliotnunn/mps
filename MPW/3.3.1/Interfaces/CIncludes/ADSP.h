...
/*
	ADSPConnectionEventProcPtr’s cannot be written in or called from a 
	high-level language without the help of mixed mode or assembly glue
	because they use the following parameter-passing convention:
...
enum  {
	uppADSPConnectionEventProcInfo = kRegisterBased|REGISTER_ROUTINE_PARAMETER(1,kRegisterA1,kFourByteCode)
};
...
#define NewADSPConnectionEventProc(userRoutine)  \
	(ADSPConnectionEventUPP) NewRoutineDescriptor((ProcPtr)(userRoutine),  \
	uppADSPConnectionEventProcInfo, GetCurrentISA())

#else
typedef ProcPtr ADSPConnectionEventUPP;
...
/*
	ADSPCompletionProcs cannot be written in or called from a high-level
...
enum  {
	uppADSPCompletionProcInfo	= kRegisterBased|REGISTER_ROUTINE_PARAMETER(1,kRegisterA0,kFourByteCode)
};
...
#define NewADSPCompletionProc(userRoutine)  \
	(ADSPCompletionUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppADSPCompletionProcInfo, GetCurrentISA())

#else
typedef ProcPtr ADSPCompletionUPP;
