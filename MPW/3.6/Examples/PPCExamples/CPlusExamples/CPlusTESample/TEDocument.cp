...
//
	TEDocument::TEDocument( short resID )	: TDocument( resID )
	{
		...
		if (!inited)
		{
			...
			TEClickLoopDesc = NewTEClickLoopProc ((TEClickLoopProcPtr) ASMCLICKLOOP);
			inited = true;
		}
...
//
	void TEDocument::AdjustTE( void )
	{
		...
		TEScroll(( te->viewRect.left - te->destRect.left ) - GetControlValue( fDocHScroll ),
				 ( te->viewRect.top - te->destRect.top ) -  (GetControlValue( fDocVScroll ) * te->lineHeight ),
				 fDocTE );
...
//
	void TEDocument::DoPaste( void )
	{
		...
		{
			if ( TEGetScrapLength() + ((*fDocTE)->teLength - ((*fDocTE)->selEnd - (*fDocTE)->selStart )) > kMaxTELength )
     ...
     pascal Boolean ASMCLICKLOOP (TEPtr pTE)
	  {
		 ...
		 (void) CallTEClickLoopProc (Old_ClickLoopRoutine, pTE);
