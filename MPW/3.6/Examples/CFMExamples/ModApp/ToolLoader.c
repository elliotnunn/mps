...
// <<< Disable the tools menu?
	...
	*fileName = '\0';	/* Empty the name */
	...
	DisposePtr((Ptr) myCPB);	// No need to check for a NULL pointer, as the Memory Mgr now does that
} /* GetToolNames */
...

{
	...
	*codeOffset = 0;
	...
	if (GetCurrentISA() == kPowerPCISA)
		myCFMArch = kPowerPCCFragArch;
	else if (GetCurrentISA() == kM68kISA)
		myCFMArch = kMotorola68KCFragArch;
	else
	...
	if (myCFMArch == kPowerPCCFragArch)
	{
	...
	if (gHasCFM) 
	{
		...
		if (parsedResource) 
		{
			...
			if (bestFit >= 0)
			{
				...
				if ((*parsedResource)->itemList[bestFit].archType == kPowerPCCFragArch)
					*codeType = kPowerPCISA | kPowerPCRTA;
				else 
			...
			DisposeHandle((Handle)parsedResource);
		}
	}
	...
	if ((err = ResError()) == noErr) {
		...
		if ((err = ResError()) == noErr) {
			...
			if (codeType == kPowerPCISA | kPowerPCRTA) {
				...
				err = GetMemFragment((Ptr)*toolHandle, 0, toolSpec->name, kPrivateCFragCopy, &connID, (Ptr*)toolStartup, errName);
				if (err) {
					DebugStr(errName);
					goto done;
				}
...
static OSErr LoadViaCFM (FSSpec *toolSpec, WindowPtr wp, ToolStartupProcPtr *toolStartup, long codeOffset)
{
	...
	err = GetDiskFragment(toolSpec, codeOffset, 0, toolSpec->name, kPrivateCFragCopy, &connID, (Ptr*)toolStartup, errName);
	if (err) {
		DebugStr(errName);
		goto done;
	}
...
// been called already
{
	...
	if (aWindow->currMenuBar) {
		DisposeHandle(aWindow->currMenuBar);
	...
	if (aWindow->toolLocation == kInResource)
		...
		DisposeHandle((Handle)aWindow->toolResource);
