{
	File:		DisAsmLookup.p
 	...
 	Copyright:	© 1984-1994, 1999 by Apple Computer, Inc.
 				All rights reserved.
...
{Dis68kOptions}
	dis68k_useClassicMnemonics = $0001;	{ (1 << 0);	(* not impl- use old-style, capitalized, less accurate mnemonics *) }
	dis68k_capitalize = $0002;	{ (1 << 1);						(* not impl- capitalize the mnemonics, but with accuracy *) }
	dis68k_use_C_Formatting = $0004;	{ (1 << 2);			(* output C-style 0xHex instead of $Hex etc *) }
	dis68k_default = 0;											(* don't use any of the above options *)

{Dis68kResults}
	dis68k_invalid =	$0000;	{ (UInt32)	(1 << 0),	(* instruction is invalid - won't run on any 68k *) }
	dis68k_68000 =		$0002;	{ (UInt32)	(1 << 1),	(* instruction runs on 68000 *) }
	dis68k_68010 =		$0004;	{ (UInt32)	(1 << 2),	(* instruction runs on 68010 *) }
	dis68k_68020 =		$0008;	{ (UInt32)	(1 << 3),	(* instruction runs on 68020 *) }
	dis68k_68030 =		$0010;	{ (UInt32)	(1 << 4),	(* instruction runs on 68030 *) }
	dis68k_68040 =		$0020;	{ (UInt32)	(1 << 5),	(* instruction runs on 68040 *) }
	dis68k_68060 =		$0020;	{ (UInt32)	(1 << 5),	(* instruction runs on 68060 *) }
											(* %%% (need this option be seperate?)
											I think the 060 is just a hypersupercharged 040
											Leave a bit for it just in case *)
	dis68k_privileged =	$0080;	{ (UInt32) (1 << 7),	(* instruction is supervisor-mode *) }
	dis68k_branch =		$0100;		{ (UInt32) (1 << 8),	(* instruction might branch *) }
	dis68k_family =		$003E;		{ (UInt32)					(* runs on any 68k cpu*) }
		{ (dis68k_68000 | dis68k_68010 | dis68k_68020 | dis68k_68030 | dis68k_68040 | dis68k_68060), }
	
	dis68k_010orLater = $003C;	{ (UInt32)	(dis68k_family ^ dis68k_68000), }
	dis68k_020orLater = $0038;	{ (UInt32)	(dis68k_family ^ (dis68k_68000 | dis68k_68010)), }
...
(** options & results **)
	...
	Dis68kOptions = integer;

	Dis68kResults = longint;

(*----------------------------------------------------------------------*)

FUNCTION Disassembler(		DstAdjust:  LongInt; 			{addr correction}
...
PROCEDURE Dis68kSetOptions(inOptions: Dis68kOptions);
