...
pascal void VActionProc(ControlHandle control, short part)
{
	...
	if ( part != 0 ) {				/* if it was actually in the control */
		...
		switch ( part ) {
			...
			case kInDownButtonControlPart:		/* one line */
				amount = 1;
				break;
			case kInPageUpControlPart:			/* one page */
			case kInPageDownControlPart:
				amount = (te->viewRect.bottom - te->viewRect.top) / te->lineHeight;
				break;
		}
		if ( (part == kInDownButtonControlPart) || (part == kInPageDownControlPart) )
...
pascal void HActionProc(ControlHandle control, short part)
{
	...
	if ( part != 0 ) {
		...
		switch ( part ) {
			case kInUpButtonControlPart:
			case kInDownButtonControlPart:		/* a few pixels */
				amount = kButtonScroll;
				break;
			case kInPageUpControlPart:			/* a page */
			case kInPageDownControlPart:
