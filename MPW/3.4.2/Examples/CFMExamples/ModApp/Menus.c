...
{
#pragma unused (userWindow)
	MenuHandle	moduleMenu = GetMHandle(kModulesMenu);
	WindowPtr	inFront = FrontWindow();
	...
	if (userWindow) {
		...
		for (item = CountMItems(moduleMenu); item > 0; item--) {
			...
			GetItem(moduleMenu, item, currItem);
...
{	
	...
	GetItem(GetMHandle(kModulesMenu), theItem, (StringPtr)&toolName);
