...
/*
	...
	that work with the new built-in type "bool"
	"Boolean" will remain an unsigned char for compatibility with source
	code written before "bool" existed.
	
	The following works with ETO #23 and CW10.  It will change in the future
	as compilers change.
*/
#ifdef __MWERKS__
	/*
		Metrowerks CW10 C++ compiler defines true and false with “Enable bool Support” is checked
		Their STL library wants them #defined instead of enums
	*/
	#if !__option(bool)
		#ifndef true
			#define true			1
		...
		#endif
	#endif
#elif defined(__MRC__) && defined(__cplusplus) && (__MRC__ >= 0x0300) && (__MRC__ < 0x0700)
	/*
		MrCpp 3.0 defines true and false when “-bool on” is used
	*/
	#if !__option(bool)
		enum {false, true};
	#endif
#else
	/*
		This is for C compilers and older C++ compilers
	*/
	enum {
		false,
		true
