...
typedef struct 
{
	...
	SInt32		dl_sap_length;			/* current length of SAP part of
										   dlsap address */
...
typedef struct 
{
	...
	UInt32	dl_xidtest_flg; 	/* if set to 1 indicates automatic
...
typedef struct 
{
	...
	UInt32	dl_token;		/* Connection response token associated
							   with the stream */
}dl_token_ack_t;
...
#define DLIOCRAW		MIOC_CMD(MIOC_DLPI,1)		/* M_DATA "raw" mode */
#define DL_IOC_HDR_INFO MIOC_CMD(MIOC_DLPI,10)		/* ??? */
