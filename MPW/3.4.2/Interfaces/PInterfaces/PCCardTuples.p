...
TYPE
	DeviceIDTuple = RECORD
		TPLDEV_TYPE_WPS_SPEED:	SInt8; (* Byte *)						{ Device Type Code: 7-4, WPS: 3,  Device Speed: 2-0}
		deviceData:				ARRAY [0..MAX_TUPLE_SIZE - 3-1] OF SInt8; (* Byte *)
	END;
...
TYPE
	ChecksumControlTuple = RECORD
		...
		TPLCKS_CS:				CHAR;									{ checksum of the region}
		reserved:				SInt8; (* Byte *)						{ padding}
	END;

{----------------    Long Link Multi-Function Tuple (06H)    ------					}
	LongLinkMFCTuple = RECORD
		TPLMFC_NUM:				SInt8; (* UInt8 *)						{ Number of sets of config registers for individual functions}
		TPLMFC_TAS1:			SInt8; (* UInt8 *)						{ CIS target address space for first function (00 = Attr, 01 = Common)}
		TPLMFC_ADDR1:			UInt32;									{ Target address of first CIS, unsigned long, lsb first}
		TPLMFC_TAS2:			SInt8; (* UInt8 *)						{ CIS target address space for second function (00 = Attr, 01 = Common)}
		TPLMFC_ADDR2:			UInt32;									{ Target address of second CIS, unsigned long, lsb first}
...
{-----------------    Link Target Tuple (13H)    ------------------							}
	LinkTargetTuple = RECORD
		TPLL_TAG:				ARRAY [0..2] OF SInt8; (* Byte *)		{ tag: should be 'C', 'I', 'S'}
	END;

{----------------    Level 1 Version Tuple (15H)    ---------------							}
	Level1VersionTuple = RECORD
		TPLLV1_MAJOR:			SInt8; (* Byte *)						{ major version number (0x04)}
		TPLLV1_MINOR:			SInt8; (* Byte *)						{ minor version number (0x01 for release 2.0 and 2.01)}
		TPLLV1_INFO:			ARRAY [0..0] OF SInt8; (* Byte *)		{ product information string, zero-terminated}
	END;

{-------------    JEDEC Identifier Tuple (18H, 19H)    ------------							}
	JEDECDeviceInfo = RECORD
		manufacturerID:			SInt8; (* Byte *)
		manufacturerInfo:		SInt8; (* Byte *)
	END;
...
{---------    Configuration Tuple (1AH)    ----------							}
	ConfigTuple = RECORD
		TPCC_SZ:				SInt8; (* Byte *)						{ size of fields byte}
		TPCC_LAST:				SInt8; (* Byte *)						{ index number of last config entry}
		TPCC_RADR:				SInt8; (* Byte *)						{ config reg's base addr in reg. space}
		reserved:				SInt8; (* Byte *)						{ padding}
		{ the following fields are of undetermined length and should be calculated at runtime}
		{UInt32			TPCC_RMSK;}
		{UInt32			TPCC_RSVD;}
...
TYPE
	DeviceGeometry = RECORD
		DGTPL_BUS:				SInt8; (* UInt8 *)						{ system bus width = 2^(n-1), n>0}
		DGTPL_EBS:				SInt8; (* UInt8 *)						{ erase block size = 2^(n-1), n>0}
		DGTPL_RBS:				SInt8; (* UInt8 *)						{ read block size = 2^(n-1), n>0}
		DGTPL_WBS:				SInt8; (* UInt8 *)						{ write block size = 2^(n-1), n>0}
		DGTPL_PART:				SInt8; (* UInt8 *)						{ partitioning size = 2^(n-1), n>0}
		DGTPL_HWIL:				SInt8; (* UInt8 *)						{ hardware interleave = 2^(n-1), n>0}
	END;
...
{-----------    Function Identification Tuple (21H)    ------------							}
	FunctionIDTuple = RECORD
		TPLFID_FUNCTION:		SInt8; (* Byte *)						{ PC card function code}
		TPLFID_SYSINIT:			SInt8; (* Byte *)						{ system initialization bit mask}
	END;
...
TYPE
	SoftwareInterleaveTuple = RECORD
		TPLSWIL_INTRLV:			CHAR;									{ software interleave}
	END;

{-------    Level 2 Version and Information Tuple (40H)    --------							}
	Level2VersionTuple = RECORD
		TPLLV2_VERS:			SInt8; (* Byte *)						{ structure version (0x00)}
		TPLLV2_COMPLY:			SInt8; (* Byte *)						{ level of compliance with the standard (0x00)}
		TPLLV2_DINDEX:			UInt16;									{ byte address of first data byte in card, LSB first}
		TPLLV2_RSV6:			SInt8; (* Byte *)						{ reserved, must be zero}
		TPLLV2_RSV7:			SInt8; (* Byte *)						{ reserved, must be zero}
		TPLLV2_VSPEC8:			SInt8; (* Byte *)						{ vendor specific byte}
		TPLLV2_VSPEC9:			SInt8; (* Byte *)						{ vendor specific byte}
		TPLLV2_NHDR:			CHAR;									{ number of copies of CIS present on this device}
		TPLLV2_OEM:				ARRAY [0..0] OF CHAR;					{ vendor of software that formatted card, zero-terminated}
		TPLLV2_INFO:			ARRAY [0..0] OF CHAR;					{ informational message about the card, zero-terminated}
		reserved:				SInt8; (* Byte *)						{ padding}
	END;
...
{ additional information for disk type:}
	FormatMemTuple = RECORD
		TPLFMT_FLAGS:			SInt8; (* Byte *)						{	various flags}
		TPLFMT_RESERVED:		SInt8; (* Byte *)						{	reserved, set to zero}
		TPLFMT_ADDRESS:			UInt32;									{	physical location at which this memory partion must be mapped}
		TPLFMT_EDCLOC:			UInt32;									{	location of error detection code, or zero if interleaved}
	END;

	FormatTuple = RECORD
		TPLFMT_TYPE:			CHAR;									{ format type code}
		TPLFMT_EDC:				CHAR;									{ error detection method and length of error detection code}
...
TYPE
	GeometryTuple = RECORD
		TPLGEO_SPT:				CHAR;									{ number of sectors per track}
		TPLGEO_TPC:				CHAR;									{ number of tracks per cylinder}
		TPLGEO_NCYL:			INTEGER;								{ number of cylinders, total}
	END;

{-----------------    Byte-Order Tuple (43H)    -------------------							}
	ByteOrderTuple = RECORD
		TPLBYTE_ORDER:			CHAR;									{ byte order code}
		TPLBYTE_MAP:			CHAR;									{ byte mapping code}
	END;
...
{----------------------    General Tuple    -----------------------							}
	TupleBody = RECORD
		...
		17: (
			tupleData:					ARRAY [0..MAX_TUPLE_SIZE - 2-1] OF SInt8; (* Byte *)
		   );
	END;
