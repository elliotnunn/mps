...
typedef STACK_UPP_TYPE(IterateFilterProcPtr)                    IterateFilterUPP;
EXTERN_API_C( IterateFilterUPP )
NewIterateFilterUPP(IterateFilterProcPtr userRoutine);
#if !OPAQUE_UPP_TYPES
  enum { uppIterateFilterProcInfo = 0x00000FC0 };  /* pascal no_return_value Func(4_bytes, 4_bytes, 4_bytes) */
  #ifdef __cplusplus
    inline IterateFilterUPP NewIterateFilterUPP(IterateFilterProcPtr userRoutine) { return (IterateFilterUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppIterateFilterProcInfo, GetCurrentArchitecture()); }
  #else
    #define NewIterateFilterUPP(userRoutine) (IterateFilterUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppIterateFilterProcInfo, GetCurrentArchitecture())
  #endif
#endif

EXTERN_API_C( void )
DisposeIterateFilterUPP(IterateFilterUPP userUPP);
#if !OPAQUE_UPP_TYPES
  #ifdef __cplusplus
    inline void DisposeIterateFilterUPP(IterateFilterUPP userUPP) { DisposeRoutineDescriptor((UniversalProcPtr)userUPP); }
  #else
    #define DisposeIterateFilterUPP(userUPP) DisposeRoutineDescriptor(userUPP)
  #endif
#endif

EXTERN_API_C( void )
InvokeIterateFilterUPP(
  const CInfoPBRec *  cpbPtr,
  Boolean *           quitFlag,
  void *              yourDataPtr,
  IterateFilterUPP    userUPP);
#if !OPAQUE_UPP_TYPES
  #ifdef __cplusplus
    inline void InvokeIterateFilterUPP(const CInfoPBRec * cpbPtr, Boolean * quitFlag, void * yourDataPtr, IterateFilterUPP userUPP) { CALL_THREE_PARAMETER_UPP(userUPP, uppIterateFilterProcInfo, cpbPtr, quitFlag, yourDataPtr); }
  #else
    #define InvokeIterateFilterUPP(cpbPtr, quitFlag, yourDataPtr, userUPP) CALL_THREE_PARAMETER_UPP((userUPP), uppIterateFilterProcInfo, (cpbPtr), (quitFlag), (yourDataPtr))
  #endif
#endif

#if CALL_NOT_IN_CARBON || OLDROUTINENAMES
    /* support for pre-Carbon UPP routines: New...Proc and Call...Proc */
    #define NewIterateFilterProc(userRoutine)                   NewIterateFilterUPP(userRoutine)
    #define CallIterateFilterProc(userRoutine, cpbPtr, quitFlag, yourDataPtr) InvokeIterateFilterUPP(cpbPtr, quitFlag, yourDataPtr, userRoutine)
